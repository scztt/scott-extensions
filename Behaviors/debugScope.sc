+ Bus {
	debugScope {
		...
		synth = { SharedOut.kr( this.index, this.kr ) }.play;
			...
			loop {
				{
					numChannels.do({
						| i |
						var aMin, aMax;
						if( Server.default.inProcess, {
							val = Server.default.getSharedControl(this.index+i);
						});
		...
		routine.play;
		w.onClose = { routine.stop; synth.free};
		w.front;
	}	
}
